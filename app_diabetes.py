...
if (selected == 'Heart Disease Prediction'):
    ...
    with col1:
        Age = st.number_input('Age', min_value=0, max__value=1000, value=40)
    with col2:
...
if st.button('Predict', key='predict_button_1'):
    ...
    if prediction ==1:
        st.markdown(f'<h4 style= "color:blue; background-color:#000; size:20px;">the model predicts you <strong>have heart disease</strong> with a probability of {probability:2f}.</h4>', unsafe_allow_html=True)
        st.status('Model pridiction compeleted')
    else:
        st.markdown(f'<h4 style="color:red; background-color:#000; size;30px">the model predicts you<strong>do not have heart desease</strong> with a probability of {1 - probability:.2f}.</h2>', unsafe_allow_html=True)
        st.status('Model Prediction Completed')
