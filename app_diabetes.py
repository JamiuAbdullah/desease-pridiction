...
if (selected == "Parkinsons Prediction"):
    ...
    with col1:
        MDVP_fo = st.number_input('MDVP:Fo(Hz)', min_value=0.00, max_value=500.000, value=95.0)
    with col2:
        MDVP_fhi = st.number_input('MDVP:Fhi(Hz)', min_value=0.00, max_value=500.000, value=95.0)
    with col3:
       MDVP_flo = st.number_input('MDVP:Flo(Hz)', min_value=0.00, max_value=500.000, value=74.0)
    with col1:
       MDVP_jitter = st.number_input('MDVP:Jitter(%)', min_value=0.001, max_value=0.00100, value=0.001)
    with col2:
        MDVP_jitter_Abs = st.number_input('MDVP:Jitter(Abs)', min_value=0.00001, max_value=0.00090, value=0.00002)
    with col3:
        MDVP_RAP = st.number_input('MDVP:RAP', min_value=0.001, max_value=0.01000, value=0.0037)
    with col1:
        MDVP_PPQ = st.number_input('MDVP:PPQ', min_value=0.001, max_value=0.01000, value=0.0071)
    with col2:
        Jitter_DDP = st.number_input('Jitter:DDP', min_value=0.001, max_value=0.01000, value=0.007)
    with col3:
        MDVP_Shimmer = st.number_input('MDVP:Shimmer', min_value=0.001, max_value=0.01000, value=0.007)
    with col1:
        MDVP_Shimmer_db = st.number_input('MDVP:Shimmer(dB)', min_value=0.01, max_value=1.000, value=0.246)
    with col2:
        Shimmer_Apq3 = st.number_input('Shimmer:APQ3', min_value=0.001, max_value=0.1000, value=0.001)
    with col3:
        Shimmer_Apq5= st.number_input('Shimmer:APQ5', min_value=0.001, max_value=0.10000, value=0.01)
    with col1:
        MDVP_Apq = st.number_input('MDVP:APQ', min_value=0.001, max_value=0.1000, value=0.0359)
    with col2:
        Shimmer_DDA = st.number_input('Shimmer:DDA', min_value=0.001, max_value=0.1000, value=0.039)
    with col3:
        NHR = st.number_input('NHR', min_value=0.001, max_value=0.1000, value=0.027)
    with col1:
        HNR = st.number_input('HNR', min_value=0.01, max_value=55.00, value=21.33)
    with col2:
       RPDE = st.number_input('RPDE', min_value=0.0001, max_value=0.900000, value=0.03173)
    with col3:
        DFA = st.number_input('DFA', min_value=0.0001, max_value=0.900000, value=0.0374)
    with col1:
        D2 = st.number_input('D2', min_value=0.001, max_value=0.900000, value=0.0375)
    with col2:
        PPE = st.number_input('PPE', min_value=0.001, max_value=0.900000, value=0.6593)

if st.button('Predict', key='predict_parkinson'):
